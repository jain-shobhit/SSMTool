function BBplotSD(nElements,sigmas)
...
for i = 1:numel(sigmas)
    strings{i} = strcat('SD_damp',num2str(sigmas(i)),'n',num2str(2*nElements),'.mat');
end
...
for i =1:numel(sigmas)
    load(strings{i})
% Reduced results
plot(SD.omega,SD.epsilon,'-','LineWidth',ssm_marker,'color',colors(i,:),'DisplayName',strcat('SSM-$$\mathcal{O}(',num2str(order),') ,\sigma = ' , num2str(10*i) , '$$'))
...
plot(fullom,fulleps,'ok', 'Markersize', coco_width,'DisplayName','Full System')
%}
cs = sigmas;
for i =1:numel(cs)
    load(strings{i})
...
for i = 1:numel(cs)
    ...
    % Reduced results
    plot(SD.omega,SD.epsilon,'-','Linewidth',ssm_sz,'color',colors(i,:),'DisplayName',strcat('SSM-$$\mathcal{O}(',num2str(order),') ,\sigma = ' , num2str(cs(end+1-i)) , '$$'))
