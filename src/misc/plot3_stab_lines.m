function plot3_stab_lines(x,y,z,S,ST,varargin)
% PLOT3_STAB_LINES This function is adapted from stab_plot in coco_plot_bd
% in coco. It plots continuation results with stability info. It
% differentiates from plot_stab_lines that it returns a 3d-plot
%
% PLOT3_STAB_LINES(X,Y,Z,S,ST,VARARGIN)
%
% X:        Data for x axis
% Y:        Data for y axis
% Z:        Data for z axis
...
while true
  ...
  if isnan(S(II))
    fig = plot3(x(II:I0), y(II:I0), z(II:I0), ST{S(II+1)+1}{:});
    stab = [stab S(II+1)+1];
  else
