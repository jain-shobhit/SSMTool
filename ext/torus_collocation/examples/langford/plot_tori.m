...
for lab = labs
  ...
  [sol data] = bvp_read_solution('tor', run, lab); % Extract trajectory segments
  N = data.nsegs;
  M = ceil(fac*size(sol{1}.xbp,1));
  clf
  hold on
  x0 = zeros(N+1,3);
  x1 = zeros(N+1,3);
  ...
  for i=1:N+1
    n       = mod(i-1,N)+1;
    XX(:,i) = sol{n}.xbp(1:M,1);
    YY(:,i) = sol{n}.xbp(1:M,2);
    ZZ(:,i) = sol{n}.xbp(1:M,3);
    x0(i,:) = sol{n}.xbp(1,:);
    x1(i,:) = sol{n}.xbp(M,:);
