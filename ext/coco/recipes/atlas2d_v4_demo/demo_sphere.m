...
%% Analogous to Example 13.1 [Not in Recipes for Continuation]
...
% In both runs below, the algorithm terminates after full coverage is
% achieved. Notably, the algorithm covers the entire sphere without
% redundancy.
...
prob = coco_set(prob, 'cont', 'h', 0.5, 'almax', 35, 'PtMX', 200); % Number of available directions = 6
coco(prob, 'sphere1', [], 2, {'x' 'y' 'z'});

prob = coco_set(prob, 'cont', 'Ndirs', 4); % Number of available directions = 4
...
coco_use_recipes_toolbox % Remove atlas2d_v4 atlas algorithm from search path
