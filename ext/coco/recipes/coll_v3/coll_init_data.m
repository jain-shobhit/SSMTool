...
% function objects.
%
% Differs from coll_v1 by including support for estimation of the
...
cntdim = dim*(NTST-1);      % Number of internal boundarie values
...
Wm          = repmat(kron(mmap, eye(dim)), [1 NTST]); % Interpolation matrix
data.Wm     = sparse(rows, cols, Wm);
x           = linspace(tm(1), tm(2), 51);
y           = arrayfun(@(x) prod(x-tm), x);
data.wn     = max(abs(y)); % Bound on mesh products
...
data.Qnum   = cntdim; % Number of rows for Jacobian of continuity conditions with respect to T and p.
