...
%DFT_ISOL2SEG   Append 'dft' instance constructed from initial data.
%
% Parse input sequence to construct toolbox data and initial solution guess
% and use this to construct an instance of 'dft'.
%
% PROB     = DFT_ISOL2SEG(PROB, OID, VARARGIN)
% VARARGIN = { @F [(@DFDX | '[]') [(@DFDP | '[]')]] T0 X0 [PNAMES] P0 ['end_dft']}
%
...
% $Id: dft_isol2orb.m 2839 2015-03-05 17:09:01Z fschild $
...
p0 = str.get;
if strcmpi(str.peek, 'end-dft') % Check for stop token
  str.skip;
end

dft_arg_check(tbid, data, t0, x0, p0);     % Validate input
data = dft_get_settings(prob, tbid, data); % Get toolbox settings
sol  = dft_init_sol(data, t0, x0, p0);     % Build initial solution guess
